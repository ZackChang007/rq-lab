...
| 分钟行情样本 | `stock/price_1m_sample.parquet` | 174,240 | 2024样本 |
| 分钟行情（50股样本） | `stock/price_1m_2024_sample.parquet` | 2,212,080 | 2024全年 ✅ 2026-05-13 新增 |
...
| 财务细分+其他因子 | 2383 | `factor/*.parquet` | ✅ 2026-05-13 完成（含68个空数据因子） |

**因子总计**: 3531 个 Parquet 文件（三时间维度完整 + 估值因子 + 技术因子 + 财务细分/其他因子）
...
| 日线行情（13品种） | `futures/price_1d.parquet` | 141,607 | ✅ 2026-05-13 新增 |
...
**总文件数**: 3531 个因子 Parquet 文件 + 其他数据文件
**总存储大小**: ~30 GB
...
6. **财务细分+其他因子**: 已全部完成，共 2383 个 ✅ 2026-05-13 完成（含68个空数据因子）
7. **补充数据下载** ✅ 2026-05-13 完成:
   - A股分钟行情样本（50股，2024全年）：2.21M 行
   - 期货日线行情（13品种）：141,607 行
   - 期权日线行情：数据为空（试用账号权限限制）
   - 期权指标：数据为空（试用账号权限限制）
8. 期权希腊值数据在试用账号下不可用
